...
export default {
  ...
  created:function(){//组件创建完毕以后
      this.$http.post('/api/getNewsList',{userId:32}).then(function (data) {//GET:请求:this.$http.get('getList').then(function (data) 
        console.log(data);
      },function (err) {
        console.log(err)
      })
  ...
  methods:{
    doSomething(){
       console.log("子组件向父组件传递信息通过自定义事件的触发来传递信息")
    }
  },
  data(){
      return {
